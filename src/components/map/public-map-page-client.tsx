...
import { useMapReports } from "../../hooks/use-public-data";
...
function getPointPosition(report: Report, reports: Report[]) {
  ...
  const left = ((lng - minLng) / lngRange) * 80 + 10;
  const top = (1 - (lat - minLat) / latRange) * 80 + 10;
...
export function PublicMapPageClient() {
  ...
  if (isError) {
    return (
      <EmptyState
        icon={MapPin}
        title="Gagal memuat peta laporan"
        description={
          error instanceof Error
            ? error.message
            : "Terjadi kesalahan saat mengambil data peta laporan."
        }
      />
    );
  }
  ...
  if (filteredReports.length === 0) {
    return (
      <EmptyState
        icon={MapPin}
        title="Belum ada laporan pada peta"
        description="Data laporan dengan koordinat lokasi belum tersedia."
      />
    );
  }
      ...
      <div className="overflow-hidden rounded-[2rem] border border-slate-100 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-5">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Cari lokasi, judul, kategori, atau status..."
        ...
        <div className="relative h-[560px] overflow-hidden bg-[#0B2D4D]">
          ...
          <div className="absolute bottom-5 left-5 rounded-2xl bg-white/10 px-4 py-3 text-sm font-semibold text-white backdrop-blur">
            {filteredReports.length} titik laporan ditampilkan
          </div>
        </div>
      </div>
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-slate-600">
                    {activeReport.address_detail || "Detail alamat belum tersedia."}
                  </p>
                </div>
              </div>
            </div>
